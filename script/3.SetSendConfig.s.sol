...
contract SetSendConfig is Script, Helper {
    ...
    function run() external {
        address endpoint = block.chainid == 421614 ? ARB_LZ_ENDPOINT : BASE_LZ_ENDPOINT; // Chain A Endpoint
        address oapp = block.chainid == 421614 ? ARB_OAPP : BASE_OAPP; // OApp on Chain A
        uint32 eid = block.chainid == 421614 ? ARB_EID : BASE_EID; // Endpoint ID for Chain B
        address sendLib = block.chainid == 421614 ? ARB_SEND_LIB : BASE_SEND_LIB; // SendLib for A → B
